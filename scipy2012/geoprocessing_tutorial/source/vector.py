...
from utilities import (verify, ugettext as safe_tr)
from utilities import ReadLayerError, WriteLayerError
from utilities import GetDataError, InaSAFEError
...
class Vector(Layer):
    ...
    def write_to_file(self, filename, sublayer=None):
        ...
        # FIXME (Ole): Work around for KML, because the driver always
        #              creates two special attributes Name and Description.
        #              If the data to be stored has e.g. three attributes
        #              'A': 'a', 'B': 'b', 'C': 'c', the KML driver will
        #              assign 'a' to 'Name', 'b' to Description and 'c' to 'A'
        #              Instead we will write the data as shp and use ogr2org
        #              to do the conversion. Sigh.
        if extension == '.kml':
            extension = '.shp'
        ...
        msg = ('Invalid file type for file %s. Only extensions '
               'sqlite, shp or kml allowed.' % filename)
        verify(extension in ['.sqlite', '.shp', '.gml', '.kml'], msg)
        ...
        if sublayer is None or extension in ['.shp', '.kml']:
        ...
        # FIXME (Ole): Workaround for KML
        if generate_kml:
            cmd = 'ogr2ogr -f KML %s %s' % (basename + '.kml', filename)
            os.system(cmd)
